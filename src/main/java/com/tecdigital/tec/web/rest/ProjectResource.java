...
/**
 * REST controller for managing {@link com.tecdigital.tec.domain.Project}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class ProjectResource {
    ...
    public List<Project> getAllProjects(@RequestParam(required = false) String filter) {
        if ("estimate-is-null".equals(filter)) {
            log.debug("REST request to get all Projects where estimate is null");
            return StreamSupport
                .stream(projectRepository.findAll().spliterator(), false)
                .filter(project -> project.getEstimate() == null)
