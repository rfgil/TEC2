  ...
  return (
    <div>
      <h2 id="project-heading">
        <Translate contentKey="tec2App.project.home.title">Projects</Translate>
        <Link to={`${match.url}/new`} className="btn btn-primary float-right jh-create-entity" id="jh-create-entity">
          <FontAwesomeIcon icon="plus" />
          &nbsp;
          <Translate contentKey="tec2App.project.home.createLabel">Create new Project</Translate>
        </Link>
      </h2>
        ...
        {projectList && projectList.length > 0 ? (
                ...
                <tr key={`entity-${i}`}>
                  ...
                  <td>
                    {project.acceptanceDate ? (
                      <TextFormat type="date" value={project.acceptanceDate} format={APP_LOCAL_DATE_FORMAT} />
                    ) : null}
                  </td>
                  <td>
                    {project.shippingDate ? <TextFormat type="date" value={project.shippingDate} format={APP_LOCAL_DATE_FORMAT} /> : null}
                  </td>
                  <td>{project.paidDate ? <TextFormat type="date" value={project.paidDate} format={APP_LOCAL_DATE_FORMAT} /> : null}</td>
                  <td>{project.closeDate ? <TextFormat type="date" value={project.closeDate} format={APP_LOCAL_DATE_FORMAT} /> : null}</td>
                  <td>{project.extraCosts}</td>
                  <td>{project.extraCostsDescription}</td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="tec2App.project.home.notFound">No Projects found</Translate>
            </div>
          )
        )}
      </div>
